...
function ServerReportPage({ server, data, notice, menu }) {
  ...
  let server_korean_name;
  switch (server) {
    case "Lufeon":
      server_korean_name = "루페온";
      break;
    case "Silian":
      server_korean_name = "실리안";
      break;
    case "Aman":
      server_korean_name = "아만";
      break;
    case "Karmain":
      server_korean_name = "카마인";
      break;
    case "Kazeros":
      server_korean_name = "카제로스";
      break;
    case "Abrelshud":
      server_korean_name = "아브렐슈드";
      break;
    case "Kardan":
      server_korean_name = "카단";
      break;
    case "Ninave":
      server_korean_name = "니나브";
      break;
  }
  ...
  return (
    <>
      <Head>
        <title>Lostar | {server_korean_name} 떠상 제보</title>
        <meta
          name="Description"
          content={`Lostar | 로스트아크 ${server_korean_name} 떠돌이 상인 제보`}
        />
      </Head>
      <MapLayer
        server={server}
...
ServerReportPage.getInitialProps = async (ctx) => {
  ...
  const result = await API.get(
    `/reports?_sort=published_at:DESC&_limit=50&_start=0&_where[0][server.name]=${encode}`
  );
