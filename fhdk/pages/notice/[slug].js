...
import MapLayer from "@components/mapLayer/notice";
...
function ServerReportPage({ server, data, notice, menu }) {
  ...
  return (
    <>
      <Head>
        <title>Lostar | 로스트아크 공지</title>
        <meta name="Description" content={`Lostar 로스트 아크 공지`} />
      </Head>
      <MapLayer
        server={server}
        data={origin}
...
ServerReportPage.getInitialProps = async (ctx) => {
  ...
  const notice = await API.get(
    `/notices?_limit=50&_start=0&_sort=published:DESC`
  );
